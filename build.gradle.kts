...
tasks {
    ...
    patchPluginXml {
        ...
        // Extract the <!-- Plugin description --> section from plugin_readme.md and provide for the plugin's manifest
        pluginDescription = providers.fileContents(layout.projectDirectory.file("plugin_readme.md")).asText.map {
            val start = "<!-- Plugin description -->"
            val end = "<!-- Plugin description end -->"

            with (it.lines()) {
                if (!containsAll(listOf(start, end))) {
                    throw GradleException("Plugin description section not found in plugin_readme.md:\n$start ... $end")
                }
