...
<#--        values (${values?join(", ")})-->
    ...
    <#if method.type == "get">
    <select id="${method.name}" <#if baseTypes?seqContains(method.result.classModel.className)>resultType="${method.result.classModel.className}"<#else>resultMap="${method.result.classModel.className}"</#if> >
        SELECT ${columns?join(", ")}
